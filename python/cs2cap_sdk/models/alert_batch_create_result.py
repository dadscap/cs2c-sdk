...
class AlertBatchCreateResult(BaseModel):
    """
    AlertBatchCreateResult
    """ # noqa: E501
    index: StrictInt = Field(description="Zero-based index of the original request item.")
    status: StrictStr = Field(description="Per-item processing status.")
    alert_id: Optional[StrictStr] = None
    code: Optional[StrictStr] = None
    message: Optional[StrictStr] = None
    __properties: ClassVar[List[str]] = ["index", "status", "alert_id", "code", "message"]

    @field_validator('status')
    def status_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['created', 'error']):
            raise ValueError("must be one of enum values ('created', 'error')")
        return value
