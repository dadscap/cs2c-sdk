# coding: utf-8

"""
    CS2Cap

    CS2Cap aggregates and analyzes Counter-Strike 2 marketplace data across multiple providers via a unified REST API.

    The version of the OpenAPI document: 1.0.0
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from cs2cap_sdk.models.batch_bids_meta import BatchBidsMeta

class TestBatchBidsMeta(unittest.TestCase):
    """BatchBidsMeta unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> BatchBidsMeta:
        """Test BatchBidsMeta
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `BatchBidsMeta`
        """
        model = BatchBidsMeta()
        if include_optional:
            return BatchBidsMeta(
                currency = '',
                requested_item_count = 56,
                found_item_count = 56,
                providers_queried = [
                    ''
                    ],
                generated_at = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f')
            )
        else:
            return BatchBidsMeta(
                currency = '',
                requested_item_count = 56,
                found_item_count = 56,
                providers_queried = [
                    ''
                    ],
                generated_at = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'),
        )
        """

    def testBatchBidsMeta(self):
        """Test BatchBidsMeta"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
