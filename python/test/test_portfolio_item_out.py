# coding: utf-8

"""
    CS2Cap

    CS2Cap aggregates and analyzes Counter-Strike 2 marketplace data across multiple providers via a unified REST API.

    The version of the OpenAPI document: 1.0.0
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from cs2cap.models.portfolio_item_out import PortfolioItemOut

class TestPortfolioItemOut(unittest.TestCase):
    """PortfolioItemOut unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> PortfolioItemOut:
        """Test PortfolioItemOut
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `PortfolioItemOut`
        """
        model = PortfolioItemOut()
        if include_optional:
            return PortfolioItemOut(
                id = '',
                item_id = 56,
                market_hash_name = '',
                phase = '',
                quantity = 56,
                source = '',
                steam_assetid = '',
                float_value = 1.337,
                paint_seed = 56,
                inspect_link = '',
                name_tag = '',
                stickers = [
                    cs2cap.models.sticker_info.StickerInfo(
                        name = '', 
                        slot = 56, 
                        wear = 1.337, )
                    ],
                charms = [
                    cs2cap.models.charm_info.CharmInfo(
                        name = '', 
                        pattern_id = 56, )
                    ],
                created_at = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f')
            )
        else:
            return PortfolioItemOut(
                id = '',
                item_id = 56,
                market_hash_name = '',
                quantity = 56,
                source = '',
                created_at = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'),
        )
        """

    def testPortfolioItemOut(self):
        """Test PortfolioItemOut"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
