...
async function main(): Promise<number> {
  ...
  try {
    response = await pricesApi.streamPricesSnapshotV1PricesPostRaw();
    for await (const line of decodeLines(response.raw.body)) {
      const item = MarketItemFromJSON(JSON.parse(line));
      processedLines += 1;
  ...
  if (collected.length === 0) {
    console.log("No rows were collected from the processed snapshot window.");
    return 0;
  }

  collected.sort((left, right) => left.lowestAsk - right.lowestAsk);
  ...
  console.log(
    renderTable(
      ["provider", "item_id", "market_hash_name", "lowest_ask", "quantity"],
      topRows.map((item) => [
        item.provider,
        item.itemId,
        item.marketHashName,
        formatMoney(item.lowestAsk),
        item.quantity,
      ]),
    ),
  );
