...
|------------- | ------------- | -------------|
| [**listSalesV1SalesGet**](SalesApi.md#listsalesv1salesget) | **GET** /v1/sales | List Recent Sales |



## listSalesV1SalesGet

> SalesHistoryResponse listSalesV1SalesGet(itemId, marketHashName, phase, providers, currency, limit)
...
import type { ListSalesV1SalesGetRequest } from 'cs2cap-sdk';
...
async function example() {
  ...
  const body = {
    ...
    limit: 56,
  } satisfies ListSalesV1SalesGetRequest;

  try {
    const data = await api.listSalesV1SalesGet(body);
    console.log(data);
  } catch (error) {
    console.error(error);
  }
